...
// Tool is a single capability the agent can invoke. Parameters returns a JSON
// Schema object describing the accepted arguments (the "parameters" field of an
// Ollama / OpenAI function definition).
type Tool interface {
	Name() string
	Description() string
...
// Spec is the wire representation of a tool advertised to the model, matching
// the Ollama / OpenAI "function" tool schema.
type Spec struct {
...
// Registry holds the set of available tools and the sandbox root they operate
// within. It is safe for sequential use by the agent loop.
...
// Execute looks up name and runs it with args. Returns an error if the tool is
// unknown so the caller can surface it back to the model.
...
// Default returns a registry with all built-in tools registered, sandboxed to
// root.
