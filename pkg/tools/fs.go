...
const maxReadBytes = 1 << 20 // 1 MiB safety cap for read_file output

// ReadFile reads a UTF-8 text file within the sandbox.
type ReadFile struct{ Root string }

func (t *ReadFile) Name() string { return "read_file" }
func (t *ReadFile) Description() string {
	return "Read the contents of a text file at the given path (relative to the project root)."
}
...
// WriteFile writes (creates or overwrites) a text file within the sandbox,
// creating parent directories as needed.
type WriteFile struct{ Root string }

func (t *WriteFile) Name() string { return "write_file" }
func (t *WriteFile) Description() string {
	return "Create or overwrite a text file with the given content (path relative to the project root). Parent directories are created automatically."
}
...
// LS lists directory entries within the sandbox.
type LS struct{ Root string }

func (t *LS) Name() string { return "ls" }
func (t *LS) Description() string {
	return "List the entries of a directory (path relative to the project root; defaults to the root)."
}
