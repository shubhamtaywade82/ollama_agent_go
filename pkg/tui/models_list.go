...
// modelItem wraps a ModelInfo for the bubbles list.
...
// ── Model picker program ───────────────────────────────────────────────────────

// modelsResult is the tea.Msg sent back when the picker finishes.
type modelsResult struct {
	selected string // empty == cancelled
	err      error
}

// modelsPicker is an independent BubbleTea model for the /models screen.
...
func newModelsPicker(models []ollama.ModelInfo, width, height int) modelsPicker {
...
// ── BubbleTea Msg plumbing ────────────────────────────────────────────────────

// modelsLoadedMsg carries the result of fetching models from Ollama.
type modelsLoadedMsg struct {
	models []ollama.ModelInfo
	err    error
}

// fetchModels is a tea.Cmd that calls the Ollama API.
func fetchModels(client *ollama.Client) tea.Cmd {
	return func() tea.Msg {
